...
async def send_music(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(callback_query.from_user.id, 'call musics mission complete ✅', reply_markup=musics)
...
async def admin_command(message: types.Message):
    ...
    if data_x[0] in data:
        ...
        await message.reply(text="qaysi servisni o'zgartirishni hohlaysiz ⬇️⬇️⬇️", reply_markup=services)
    else:
...
async def admin_change(message: types.Message):
    ...
    if data_2[0] in data_1:
        if message.text == 'add musics':
            await message.reply("qaysi qo'shiqchini qo'shmoqchisiz 🤔⬇️: ", reply_markup=add_musics)
        elif message.text == 'add videos':
            await message.reply(text="video nomini kiritng(film, serial, short video)⬇️⬇️⬇️: ")
        elif message.text == 'change admins':
            await message.reply(text="qo'shiladigan admin chat_idsini kiriting ⬇️⬇️⬇️:")
...
async def show_menu(message: types.Message):
    await message.answer("Menulardan birini tanlang⬇️⬇️⬇️:", reply_markup=keyboard)
...
async def menu_ins(callback_query: types.CallbackQuery):
    ...
    if callback_query.data == "musics":
        await callback_query.answer('✅')
        await bot.send_message(callback_query.from_user.id, 'choose musics ⬇️⬇️⬇️', reply_markup=musics)
    else:
        await bot.send_message(callback_query.from_user.id, 'choose video ⬇️⬇️⬇️', reply_markup=videos)


@dp.message_handler(lambda message: message.text == 'Category')
async def show_category(message: types.Message):
    await message.answer("Categoriyalardan birini tanlang⬇️⬇️⬇️:", reply_markup=category)
...
async def category_m(callback_query: types.CallbackQuery):
    ...
    if callback_query.data == "music category":
        await bot.send_message(callback_query.from_user.id, 'choose music category ⬇️⬇️⬇️:', reply_markup=musics_category)
    else:
        await bot.send_message(callback_query.from_user.id, 'choose video ⬇️⬇️⬇️:', reply_markup=videos_category)


@dp.message_handler(lambda message: message.text == 'back')
async def back_x(message: types.Message):
    await message.answer("Menulardan birini tanlang ⬇️⬇️⬇️:", reply_markup=menu_keyword)
